...
;00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F 10 11 12 13	HEX
...
loop:					;The main game loop
	...
	jz loop				;Timer is not running, skip incrementing

	addi r2, 10			;Increase tenths

	mov ra, r2			;Copy tenths to scratchpad
	subi ra, 100			;Check maximum value
	jz increment_seconds		;Maximum value reached, increase seconds
...
:increment_seconds
	...
	jz increment_minutes		;Maximum value reached, inscrease minutes
	jmp loop			;Draw another frame
...
:increment_minutes
	...
	mov ra, r0			;Copy minutes to scratchpad
	subi ra, 60			;Check maximum value
	jz reset_timer_call		;Maximum value reached, reset timer
	jmp loop			;Draw another frame

:reset_timer_call
	call reset_timer		;Reset values to defaults
...
:process_input				;Process pressed keys
	...
	jz clear_pad_flag		;No keys are pressed, clear flag
...
:check_pad_flag
	...
	jz process_a			;Process pressed keys
...
:process_a
	...
	jz process_b			;A is not pressed, ckeck B
...
:process_b
	...
	jz fret				;B is not pressed, break subroutine
...
:wait_frames				;Wait for the required number of frames
	...
	jz fret				;Counter is zero, break subroutine
