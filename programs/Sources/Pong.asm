...
:move_p1_up
	...
	jz move_p1_down		;Check if Player 1 paddle is in the minimal allowed position
	mov rb, re			;Copy Pad 1 state to scratchpad
	andi rb, 1			;Check if UP is pressed on Pad 1
	jz move_p1_down		;UP is not pressed
...
:move_p1_down
	...
	jz move_p2_up			;DOWN is not pressed
	addi r1, 4			;Increase Y coordinate

:move_p2_up
	addi r4, 0			;Update flags
	jz move_p2_down		;Check if Player 2 paddle is in the minimal allowed position
	mov rb, rf			;Copy Pad 2 state to scratchpad
	andi rb, 1			;Check if UP is pressed on Pad 2
	jz move_p2_down		;UP is not pressed
...
:move_p2_down
	...
	jz draw_ball			;DOWN is not pressed
	addi r4, 4			;Increase Y coordinate

:draw_ball
	spr #0402			;4x4 px sprite
	drw r6, r7, ball		;Draw ball
	jc change_direction		;Collision happened, change ball direction
...
:change_direction
	...
	jc xor_direction		;Direction should be changed

	mov ra, r6			;Copy ball X position to scratchpad
	subi ra, 250			;Check if ball X is higher then 50 px
	jc move_ball_x			;Skip direction change
...
:move_ball_x
	...
	jz move_ball_left		;Movement is set to left
...
:check_goal
	...
	jc p2_scored			;Player 2 scored
	mov rb, r6			;Copy ball X position
	subi rb, 312			;Remove playfield offset
	jc game_loop			;Draw new frame, otherwise Player 1 scored
...
:wait_frames				;Wait for required number of frames // void WaitFrames(ushort FramesToWait)
	...
	jz fret			;Counter is zero, jump to return function
